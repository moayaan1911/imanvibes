...
type SearchableGridProps = {
  ...
  itemLabel: string;
  placeholder: string;
  categoryStyle?: "default" | "mood-grid" | "dua-grid";
...
const defaultMoodTileStyle: MoodTileStyle = {
...
const moodTileStyles: Record<string, MoodTileStyle> = {
  Sad: {
    icon: MdWaterDrop,
    cardClassName: "bg-[#f3ede4] hover:bg-[#ede7df]",
    iconClassName: "text-[var(--sage-500)]",
...
const duaCategoryIcons: Record<string, IconType> = {
...
export default function SearchableGrid({
  ...
  const singularItemLabel = itemLabel.replace(/s$/, "");

  const renderCategoryCard = (cat: SearchCategory) => {
    if (categoryStyle === "mood-grid") {
      const {
        ...
        titleClassName,
        metaClassName,
      } = moodTileStyles[cat.name] ?? defaultMoodTileStyle;

      return (
        <Link
          key={cat.href}
          href={cat.href}
          className={`group relative flex aspect-square cursor-pointer flex-col justify-between overflow-hidden rounded-[20px] p-3.5 transition-colors ${cardClassName}`.trim()}
        >
          <Icon className={`text-[1.25rem] ${iconClassName}`.trim()} />
          <div className="mt-3">
            <p
              className={`text-[0.94rem] font-semibold leading-tight ${titleClassName}`.trim()}
            >
              {cat.name}
            </p>
            {showCategoryCounts ? (
              <p className={`mt-1 text-[11px] leading-4 ${metaClassName}`.trim()}>
                {cat.count} {cat.count === 1 ? singularItemLabel : itemLabel}
              </p>
            ) : null}
          </div>
        </Link>
    ...
    if (categoryStyle === "dua-grid") {
      const Icon = duaCategoryIcons[cat.name] ?? MdAutoStories;
    ...
    return (
      ...
      >
        <p className="text-sm font-semibold text-[var(--ink-900)]">{cat.name}</p>
        {showCategoryCounts ? (
