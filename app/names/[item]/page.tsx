...
export async function generateMetadata({
  ...
  const entry = getAllahNameById(item);
  ...
  return createSeoMetadata({
    title: `${entry.transliteration} - 99 Names`,
    description: summarizeText(`${entry.transliteration}: ${entry.meaning}`, 155),
    path: `/names/${entry.id}`,
    imagePath: `/og/names/${entry.id}/opengraph-image`,
    keywords: ["99 Names of Allah", "Asma ul Husna", "Islamic reflection"],
  });
}

export default async function NameItemPage({
  params,
}: {
  params: Promise<{ item: string }>;
}) {
  const { item } = await params;
  const currentEntry = getAllahNameById(item);

  if (!currentEntry) {
    notFound();
  }

  const currentPath = `/names/${currentEntry.id}`;
  const structuredData = [
    getWebPageJsonLd({
      title: currentEntry.transliteration,
      description: summarizeText(`${currentEntry.transliteration}: ${currentEntry.meaning}`, 155),
      path: currentPath,
    }),
    getBreadcrumbJsonLd([
      { name: "Home", path: "/" },
      { name: "99 Names", path: "/names" },
      { name: currentEntry.transliteration, path: currentPath },
    ]),
    getNameJsonLd({
      urlPath: currentPath,
